...
def plot_dual(original, predict, hist, modelname, title, xlabel, ylabel):
    ...
    plt.subplot(1, 2, 1)
    ax = sns.lineplot(
        x=original.index, y=original[0], label="True History", color="royalblue"
    )
    ax = sns.lineplot(
        x=predict.index, y=predict[0], label=f"Prediction ({modelname})", color="tomato"
    )
...
def plot_multi(result, title, feature):
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(go.Scatter(x=result.index, y=result[0], mode="lines", name="Train"))
    )
    fig.add_trace(go.Scatter(x=result.index, y=result[1], mode="lines", name="Test"))
    fig.add_trace(
        go.Scatter(x=result.index, y=result[2], mode="lines", name="Forecast")
    )
    fig.add_trace(
        go.Scatter(go.Scatter(x=result.index, y=result[3], mode="lines", name="Actual"))
    )
    fig.update_layout(
        xaxis=dict(
            showline=True,
    ...
    annotations.append(
        dict(
            ...
            yanchor="bottom",
            text=title,
            font=dict(family="Rockwell", size=16, color="white"),
            showarrow=False,
        )
    )

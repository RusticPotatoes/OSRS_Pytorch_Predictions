...
def save_plot_to_png(input_plot, filename, folderpath):
    ...
    ):  # check subdir path and make it, append the subdir to img_dir
        if not os.path.exists(folderpath):
            os.makedirs(folderpath, mode)
    # print(os.path.join(folderpath, filename))
    input_plot.savefig(
        os.path.join(folderpath, filename.replace(" ", "_"))
    ...
    for i in range(fut_pred):
        ...
        with torch.no_grad():
            ...
            torch_tensor_dataset = torch.cat(
                (torch_tensor_dataset, (results)[np.newaxis, :]), dim=1
            )

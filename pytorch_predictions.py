...
def main():
    ...
    # List of your known project folders
    project_folders = [
        models_dir,
        features_dir,
        img_dir,
    ...
    for item_to_predict in items_to_predict_df_names:
        for feature_to_predict_name in features_to_predict_df_names:
            output_model = os.path.join(
                models_dir, f"{item_to_predict}_{feature_to_predict_name}.pth"
            )
            ...
            ######################
            # for each item
            apitimeseries = PricesAPI(
                "OSRS_PYTORCH_PREDICTIONS", "OSRS_PYTORCH_PREDICTIONS"
            )
            runelite_timeseries_df = apitimeseries.timeseries_df(
                "5m", utils.getIDFromName(item_mapping_df, item_to_predict)
            )
            # add the name column back
            runelite_timeseries_df["name"] = item_to_predict
            # epoch unix to datetime
            runelite_timeseries_df["timestamp"] = pd.to_datetime(
                runelite_timeseries_df["timestamp"], unit="s"
            )
            # fix 0's to ba nans
            ...
            price_all = pd.DataFrame(
                runelite_timeseries_df, columns=features_to_predict_df_names
            )
            ...
            if save_img:
                ...
                item_to_predict_dataset_plot = plot.plot_single(
                    ...
                    xaxisticks=(
                        range(0, price.shape[0], 30)
                    ),  # from 0 to shape range (300) increment 30
            ...
            # split the data up
            x_train, y_train, x_test, y_test = utils.split_data_3d(
                price_scaled, lookback
            )
            if useSavedModels:
            ...
            if not useSavedModels:
                # train predictions
                y_train_pred_lstm, hist_lstm = trainers.train(
                    model_lstm,
                    criterion,
                    optimiser_lstm,
                    num_epochs,
                ...
                y_train_pred_lstm_inverse = utils.unnormalizer(
                    y_train_pred_lstm_detatched, price_std, price_mean
                )
                y_train_pred_lstm_inverse_df = pd.DataFrame(y_train_pred_lstm_inverse)
                y_train_lstm_detatched = y_train_lstm.detach().numpy()
                y_train_lstm_inverse = utils.unnormalizer(
                    y_train_lstm_detatched, price_std, price_mean
                )
                y_train_lstm_inverse_df = pd.DataFrame(y_train_lstm_inverse)
                # plot original and prediction graphs with training loss
                if save_img:
                    dual_plot_lstm = plot.plot_dual(
                        original=y_train_lstm_inverse_df,
                        predict=y_train_pred_lstm_inverse_df,
                        hist=hist_lstm,
                        modelname="LSTM",
                        title=f"{item_to_predict}",
                        xlabel="Date",
                        ylabel=f"Gold (GP) [{feature_to_predict_name}]",
                    )
                    utils.save_plot_to_png(
                        dual_plot_lstm,
                        f"lstm_dual_{item_to_predict}_{feature_to_predict_name}.png",
                        training_dir,
                    )
            ...
            y_test_pred_lstm_inverse = utils.unnormalizer(
                y_test_pred_lstm_detatched, price_std, price_mean
            )
            y_test_pred_lstm_inverse_df = pd.DataFrame(y_test_pred_lstm_inverse)
            y_test_lstm_detatched = y_test_lstm.detach().numpy()
            y_test_lstm_inverse = utils.unnormalizer(
                y_test_lstm_detatched, price_std, price_mean
            )
            ...
            if not useSavedModels:
                ...
                trainScore_lstm = math.sqrt(
                    mean_squared_error(
                        y_train_lstm_detatched[:, 0], y_train_pred_lstm_detatched[:, 0]
                    )
                )
                print(
                    f"{item_to_predict} {feature_to_predict_name}: "
                    + "%.2f RMSE" % (trainScore_lstm)
                )
            testScore_lstm = math.sqrt(
                mean_squared_error(
                    y_test_lstm_detatched[:, 0], y_test_pred_lstm_detatched[:, 0]
                )
            )
            print(
                f"{item_to_predict} {feature_to_predict_name}: "
                + "%.2f RMSE" % (testScore_lstm)
            )
            ...
            z_forecast_lstm_scaled_reshaped_3d = model_lstm.forecast(
                dataset=price_scaled_reshaped_3d, lookback=lookback, fut_pred=fut_pred
            )  # model_lstm.forecast(dataset=z_future_tensor,lookback=lookback,fut_pred=fut_pred)
            ...
            z_forecast_lstm_inverse = utils.unnormalizer(
                z_forecast_lstm_scaled_reshaped_3d[0, -fut_pred:, :],
                price_std,
                price_mean,
            )
            ...
            if not useSavedModels:
                ...
                if verbose:
                    print(
                        f"trainPredictPlot: {lookback}: {len(y_train_pred_lstm_inverse)+lookback} , :"
                    )
                trainPredictPlot[
                    lookback : len(y_train_pred_lstm_inverse) + lookback, :
                ] = y_train_pred_lstm_inverse
            ...
            if not useSavedModels:
                if verbose:
                    print(
                        f"test predict plot: {len(y_train_pred_lstm_inverse)+lookback} : {len(price)} , :"
                    )
                testPredictPlot[
                    len(y_train_pred_lstm_inverse) + lookback : len(price), :
                ] = y_test_pred_lstm_inverse
            if useSavedModels:
                if verbose:
                    print(f"test predict plot: {lookback} : {len(price)} , :")
            ...
            if verbose:
                print(
                    f"forecast predict plot: {price.shape[0]} : {price.shape[0]+fut_pred} , :"
                )
            forecastPredictPlot[
                price.shape[0] : price.shape[0] + fut_pred, :
            ] = z_forecast_lstm_inverse
            ...
            test_pred_fig = plot.plot_multi(
                result,
                title=f"{item_to_predict} (LSTM)",
                feature=feature_to_predict_name,
            )
            ...
            output_img = os.path.join(
                forecast_dir,
                (
                    f"lstm_dual_{item_to_predict}_{feature_to_predict_name}_train_test_forcast.png"
                ).replace(" ", "_"),
            )
            # output_img_str = f'"{output_img}"'
            # test_pred_fig.write_image(engine="kaleido", file=output_img_str)
            pio.write_image(test_pred_fig, output_img)
